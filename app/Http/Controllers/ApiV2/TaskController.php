...
class TaskController extends Controller
{
    ...
    public function tasks_list(Request $request){
    	...
    	if(!$consignment){
	        ...
	        $feedback['response']['tasks']   =  $this->completedTaskList($user_id);

    	}else{

    		// Get Task Locations
    		$tasks = $this->getTasksList($consignment->id, $consignment->type, $user_id);
					...
					try {
						DB::beginTransaction();
							...
							if($startTask == 1){
						        ...
						        $message[]       =  'The task is not started';
    ...
    public function consignments(Request $request){
    	...
    	$consignments = Consignment::orderBy('id', 'desc')
