    ...
    public function pick_up(Request $request)
    {
        ...
        if ($request->has('order_by')) {
            switch ($request->order_by) {
                case 'address':
                    $query->orderBy('pickup_locations.address1', $orderBy);
                    break;
                case 'zone':
                    $query->orderBy('zones_p.id', $orderBy);
                    break;
                case 'city':
                    $query->orderBy('cities_p.id', $orderBy);
                    break;
                default:
        ...
        return view('consignments.pick-up.index', compact('sub_orders', 'pickupman', 'merchants', 'stores', 'zones', 'cities'));
    }
