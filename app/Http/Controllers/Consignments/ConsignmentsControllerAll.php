            ...
            foreach ($tasks as $task) {
                switch ($task->task_type_id) {
                    ...
                    case 2:
                        ...
                        // Send SMS
                        $sms = "Dear ".$task->suborder->order->delivery_name.", Your product is on the way. Your FastBazzar security code is: ".$task->otp;
                        $this->sendCustomMessage($task->suborder->order->delivery_msisdn, $sms, $task->suborder->id);
                        break;
                    case 4:
                        $this->suborderStatus($task->sub_order_id, '36');
                        $sms = "Dear Seller, Your product is being return. Your FastBazzar security code is: ".$task->otp;
                        $this->sendCustomMessage($task->suborder->order->products->pickup_location->msisdn, $sms, $task->suborder->id);
                        break;
                }
