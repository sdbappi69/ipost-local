    ...
    <div class="portlet-body util-btn-margin-bottom-5">
      <table class="table table-bordered table-hover" id="example0">
        ...
        <tbody>
            ...
            <td>
                @if($t->bank_transection_doc_id == 0 and $t->status != 3)
                <button title="upload transaction document" onclick="upload_doc({{$t->id}})" type="button" class="btn btn-primary" >
                  <i class="fa fa-upload"></i>
              </button>
              @endif
              @if(Auth::user()->hasRole('head_of_accounts'))
              @if($t->bank_transection_doc_id != 0 and !$t->bank_transection_id and $t->status != 3)
