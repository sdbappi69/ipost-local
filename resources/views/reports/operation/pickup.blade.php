...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span>Pickup Failed Report</span>
        </li>
    </ul>
</div>
<!-- END PAGE BAR -->
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> Pickup Failed Report</h1>
<!-- END PAGE TITLE-->
<!-- END PAGE HEADER-->
...
<div class="col-md-12">
    ...
    <div class="portlet light tasks-widget bordered">
        <div class="portlet-title">
            <div class="caption">
                <i class="icon-edit font-dark"></i>
                <span class="caption-subject font-dark bold uppercase">Pickup Failed</span>
            </div>
            <div class="tools">
        ...
        <div class="portlet-body util-btn-margin-bottom-5">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $pickupFailed->order_id }}</td>
                        <td>{{ $pickupFailed->order_created_at }}</td>
                        <td>{{ $pickupFailed->merchant_name }}</td>
                        <td>{{ $pickupFailed->merchant_order_id }}</td>
                        <td>{{ $pickupFailed->current_status }}</td>
                        <td>{{ $pickupFailed->failed_reason }}</td>
                        <td>{{ $pickupFailed->remarks }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <div class="pagination pull-right">
                {{ $pickupFailedData->appends($inputs)->render() }}
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document ).ready(function() {
        ...
        highlight_nav('operation-pickup', 'operation');
    ...
    $(".export-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('operation/failed-pickupexport/xls') }}").submit();
    });
    $(".filter-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('operation/failed-pickup') }}").submit();
    });
