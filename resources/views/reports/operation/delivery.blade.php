...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span>Delivery Failed Report</span>
        </li>
    </ul>
</div>
<!-- END PAGE BAR -->
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> Delivery Failed Report</h1>
<!-- END PAGE TITLE-->
<!-- END PAGE HEADER-->
...
<div class="col-md-12">
    ...
    <div class="portlet light tasks-widget bordered">
        <div class="portlet-title">
            <div class="caption">
                <i class="icon-edit font-dark"></i>
                <span class="caption-subject font-dark bold uppercase">Delivery Failed</span>
            </div>
            <div class="tools">
        ...
        <div class="portlet-body util-btn-margin-bottom-5">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $deliveryFailed->order_id }}</td>
                        <td>{{ $deliveryFailed->order_created_at }}</td>
                        <td>{{ $deliveryFailed->merchant_name }}</td>
                        <td>{{ $deliveryFailed->merchant_order_id }}</td>
                        <td>{{ $deliveryFailed->current_status }}</td>
                        <td>{{ $deliveryFailed->pickup_hub }}</td>
                        <td>{{ $deliveryFailed->delivery_hub }}</td>
                        <td>{{ $deliveryFailed->failed_reason }}</td>
                        <td>{{ $deliveryFailed->remarks }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <div class="pagination pull-right">
                {{ $deliveryFailedData->appends($inputs)->render() }}
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document ).ready(function() {
        ...
        highlight_nav('operation-delivery', 'operation');
    ...
    $(".export-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('operation/failed-deliveryexport/xls') }}").submit();
    });
    $(".filter-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('operation/failed-delivery') }}").submit();
    });
