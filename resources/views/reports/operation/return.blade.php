...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span>Return Failed Report</span>
        </li>
    </ul>
</div>
<!-- END PAGE BAR -->
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> Return Failed Report</h1>
<!-- END PAGE TITLE-->
<!-- END PAGE HEADER-->
...
<div class="col-md-12">
    ...
    <div class="portlet light tasks-widget bordered">
        <div class="portlet-title">
            <div class="caption">
                <i class="icon-edit font-dark"></i>
                <span class="caption-subject font-dark bold uppercase">Return Failed</span>
            </div>
            <div class="tools">
        ...
        <div class="portlet-body util-btn-margin-bottom-5">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $returnFailed->order_id }}</td>
                        <td>{{ $returnFailed->order_created_at }}</td>
                        <td>{{ $returnFailed->merchant_name }}</td>
                        <td>{{ $returnFailed->merchant_order_id }}</td>
                        <td>{{ $returnFailed->current_status }}</td>
                        <td>{{ $returnFailed->pickup_hub }}</td>
                        <td>{{ $returnFailed->delivery_hub }}</td>
                        <td>{{ $returnFailed->failed_reason }}</td>
                        <td>{{ $returnFailed->remarks }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <div class="pagination pull-right">
                {{ $returnFailedData->appends($inputs)->render() }}
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document ).ready(function() {
        ...
        highlight_nav('operation-return', 'operation');
