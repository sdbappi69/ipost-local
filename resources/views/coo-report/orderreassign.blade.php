...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <span>Order Reassign</span>
        </li>
    </ul>
</div>
<!-- END PAGE BAR -->
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> Order Reassign
    <small> view</small>
</h1>
<!-- END PAGE TITLE-->
<!-- END PAGE HEADER-->
...
<div class="col-md-12">
    ...
    <div class="portlet light tasks-widget bordered">
        <div class="portlet-title">
            <div class="caption">
                <i class="icon-edit font-dark"></i>
                <span class="caption-subject font-dark bold uppercase">Order Reassign</span>
            </div>
            <div class="tools">
        ...
        <div class="portlet-body util-btn-margin-bottom-5">
            <table class="table table-bordered table-hover" id="example0">
                ...
                <tbody>
                    @foreach($order_reassign as $reassigned)
                    <?php
                    $sub_order_status = '';
                    ...
                    ?>
                    <tr>
                        <td>{{ $reassigned['unique_order_id'] }}</td>
                        <td>{{ $reassigned['merchant_order_id'] }}</td>
                        <td>{{ $reassigned['merchant_name'] }}</td>
                        <td>{{ $reassigned['store_name'] }}</td>
                        <td>{{ $reassigned['delivery_name'] }}</td>
                        <td>{{ $reassigned['created_at'] }}</td>
                        <td>{{ $reassigned['pickup_hub'] }}</td>
                        <td>{{ $reassigned['delivery_hub'] }}</td>
                        <td>{{ $reassigned['zone'] }}</td>
            ...
            <div class="pagination pull-right">
                {{ $order_reassign->appends($inputs)->render() }}
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document ).ready(function() {
        ...
        highlight_nav('order-reassign', 'order-report');
    ...
    $(".export-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('order-reassign-export/xls') }}").submit();
    });
    ...
    $(".filter-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('order-reassign') }}").submit();
    });
