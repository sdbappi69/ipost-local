...
<!-- END PAGE BAR -->
<!-- BEGIN PAGE TITLE-->
<h1 class="page-title"> Order Completed
    <small> view</small>
</h1>
<!-- END PAGE TITLE-->
<!-- END PAGE HEADER-->
...
<div class="col-md-12">
    ...
    <div class="portlet light tasks-widget bordered">
        <div class="portlet-title">
            <div class="caption">
                <i class="icon-edit font-dark"></i>
                <span class="caption-subject font-dark bold uppercase">Order Completed</span>
            </div>
            <div class="tools">
        ...
        <div class="portlet-body util-btn-margin-bottom-5">
            <table class="table table-bordered table-hover" id="example0">
                ...
                <tbody>
                    ...
                    ?>
                    <tr>
                        <td>{{ $completed['unique_order_id'] }}</td>
                        <td>{{ $completed['merchant_order_id'] }}</td>
                        <td>{{ $completed['merchant_name'] }}</td>
                        <td>{{ $completed['store_name'] }}</td>
                        <td>{{ $completed['delivery_name'] }}</td>
                        <td>{{ $completed['created_at'] }}</td>
                        <td>{{ $completed['pickup_hub'] }}</td>
                        <td>{{ $completed['delivery_hub'] }}</td>
                        <td>{{ $completed['zone'] }}</td>
            ...
            <div class="pagination pull-right">
                {{ $order_completed->appends($inputs)->render() }}
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    $(document ).ready(function() {
        ...
        highlight_nav('order-completed', 'order-report');
    ...
    $(".export-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('order-completed-export/xls') }}").submit();
    });
    $(".filter-btn").click(function(e){
        // alert(1);
        e.preventDefault();
        $('#filter-form').attr('action', "{{ secure_url('order-completed') }}").submit();
    });
