<div class="row">
                            ...
                            <tbody>
                                @foreach($charges->order_detail as $row)
                                    <tr>
                                        ...
                                        <td class="numeric">{{ $row->product_unit_price }}</td>
                                        <td class="numeric">{{ $row->product_unit_delivery_charge }}</td>
                                        <td class="numeric">{{ $row->product_quantity }}</td>
                                        <td class="numeric">{{ $row->product_total_price }}</td>
                                        <td class="numeric">{{ $row->product_delivery_charge }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                ...
                                <tr>
                                    ...
                                    <td style="padding-right:5px;">
                                        {!! Form::text('amount', $order->amount, ['class' => 'form-control', 'placeholder' => 'BDT', 'required' => 'required', 'id' => 'amount']) !!}
    ...
    <script type="text/javascript">
        ...
        $(document ).ready(function() {
            ...
            @else
                var amount = $('#order_product_charge').val();
            @endIf
            collectableAmount(amount, $('#delivery_payment_amount').val(), $('#include_delivery:checkbox:checked').val());
        });
